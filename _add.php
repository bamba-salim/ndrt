...
if (isset($_GET['action'])) :
  switch ($_GET['action']):
    ...
    case 'user': // create user -> done
      ...
      //todo
      $user = [];
      // $m = $USER->getCount("WHERE mail = :mail'", array(':mail' => $mail));
      // $m = $USER->getCount("WHERE mail = :username'", array(':username' => $username));
      $user['username'] = $FORM->usedInput($username, $u, REGEX::USERNAME) ? $errors['username'] = "Pseudo invalide ou déjà utiliser" : $username;
      $user['role'] = $FORM->newInput($role, REGEX::USERNAME) ? USER::CLIENT_ID : $role;
      $user['nom'] = $FORM->newInput($nom, REGEX::WORDS) ? $errors['nom'] = "Nom invalide" : $nom;
      $user['prenom'] = $FORM->newInput($prenom, REGEX::WORDS) ? $errors['prenom'] = "Prenom invalide" : $prenom;
      $user['mail'] = $FORM->usedInput($mail, $m, REGEX::MAIL) ? $errors['mail'] = "Mail invalide ou déjà utilisé" : $mail;
      $user['password'] = $FORM->passInput($passFST, $passSND) ? $errors['pass'] = "Mot de pass invalide ou non identique" : password_hash($passFST, PASSWORD_BCRYPT, $FORM::HASH);
      if (!empty($errors)) {
        $_SESSION['errors'] = $errors;
        $_SESSION['inputs'] = $_POST;
        ...
        $user['ref'] = $ref;
        $_SESSION['user'] = $user;
        $_SESSION['success'] = "Binvenue et voici mon tableau de bord ou vous retrouverez toutes vos informations";


        $USER->creatUser($user);
        DB()->query2(USER::CREATE, array(':role' => $role, ':ref' => $ref, ':username' => $username, ':nom' => $nom, ':prenom' => $prenom, ':mail' => $mail, ':password' => $password));
        //header('location: ./_log?action=up');
      }
      break;
